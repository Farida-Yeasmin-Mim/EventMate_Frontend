...
import '../../event/filter/DecorationFilter.css';
...
const DecorationFilter = () => {
  ...
  const handleChangeChecked = (id) => {
    ...
    console.log(changeCheckedDecorations);
  ...
  const applyFilters = () => {
    ...
    if (decorationsChecked.length) {
      updatedList = updatedList.filter((item) =>
        ...
        // item.decoration.includes(decorationsChecked)
      );
    }
  ...
  useEffect(() => {
    applyFilters();
  }, [selectedRating, selectedCategory, selectedPrice, searchInput]);
